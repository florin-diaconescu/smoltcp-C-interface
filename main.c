...
// 		for (i = 0; i < k; i++) {
// 			nb = queue[i];
// 			uknetdev_input(lo_stack, nb->data);
// 			prepare_packet(nb);
// 			// fprintf(stderr, "%u\n",  nb->len);
// 			uknetdev_output(dev, nb);
...
// 	}
	...
	smoltcp_stack_t *lo_stack = init_uknetdev_stack();
	add_ipv4_address(lo_stack, 127, 0, 0, 1, 8);
	build_interface(lo_stack);

	while (1) {
		ret = smoltcp_uk_recv(lo_stack);
		if (ret) return 0;
		fprintf(stderr, "Handler done!");
		for (i = 0; i < k; i++) {
			nb = queue[i];
			smoltcp_uk_send(lo_stack, nb->data);
			fprintf(stderr, "Uknetdev_outputed!");
		}
	}
